...
Collectibles::Collectibles(int w, int h) {
...
void Collectibles::timeTick() {
    time += 1;
}

bool Collectibles::off_screen() {
    return not_on_screen;
}

void Collectibles::fly() {
    if(moverRect.x < 500) 
    {
        v_y = y_o - (g*time);
...
bool Collectibles::getBurst()
{
    return burst;
}

void Collectibles::setBurst()
{
    burst = true;
}

SDL_Rect Collectibles::getLocation()
{
    return moverRect;
}

void Collectibles::operator--(){}

bool Collectibles::hit(int MouseX, int MouseY)
{   
